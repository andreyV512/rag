...
namespace RectifierNS
{
    public class Crc16
    {
        ...
        public static ushort Compute(byte[] _data)
        {
            return(Compute(_data, _data.Length));
        }
        public static ushort Compute(byte[] _data, int _length)
        {
            ushort crc = 0xFFFF;
            for(int i=0;i<_length;i++)
                crc = (ushort)((crc >> 8) ^ CrcTable[(crc ^ _data[i]) & 0xFF]);
            return crc;
        }
        public static void Add(byte[] _data)
        {
        ...
        {
            ushort crc = Compute(_data, _data.Length - 2);
            //byte a1= Convert.ToByte(crc >> 8);
            //byte a2= Convert.ToByte(crc & 0xff);
